...
class Unicast extends RecOP_Controller {
	...
	public function unimail()
	{
		$sent = 0;
		$this->db->from($this->table_name);
//		$this->db->where('memberid >=', 'M0002012320');
		$this->db->where_in('pcode1', array('S0120377'));
		$query = $this->db->get();
		$result = $query->result_array();
die(var_export($result));
		...
		foreach($result as $record) {
			$this->mail($record);
			$sent++;
			echo "S\t" . $record['username'] . "\t" . $record['corpname'] . "\t" . $record['c_email'] . "<br>";
		}
	}
