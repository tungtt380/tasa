...
{
	protected $form_prefix   = 'en/entry';		// フォーム名
...
//      'd_fullkana'    => 'trim|required|xss_clean|prep_kana|valid_kana',
        ...
        'q_boothcount1' => 'trim|required|xss_clean|is_natural',
        'q_boothcount2' => 'trim|xss_clean|is_natural',
        'q_boothcount3' => 'trim|xss_clean|is_natural',
        'q_booth1'      => 'trim|required|xss_clean|is_select_natural',
	...
	protected function setup_form(&$data)
	{
		...
		$data['countrycode'] = $this->country_model->get_dropdown('en');
		...
		$data['booth'] = $this->booth_model->get_dropdown(FALSE,FALSE,FALSE,'en');
		$data['boothgroup'] = $this->booth_model->get_dropdown(TRUE,FALSE,FALSE,'en');
        ...
        if ($query->num_rows() > 0) {
            foreach($query->result_array() as $row) {
                if (strlen($row['spaceabbr']) == 1) {
                    ...
                    if ($row['inventory'] == 1) {
                        $waiting[$row['spaceid']] = $row['spaceabbr'] . 'Space'; //'スペース';
                    }
                    if ($row['inventory'] == 9) {
                        $soldout[$row['spaceid']] = $row['spaceabbr'] . 'Space'; //'スペース';
                    }
                }
            }
        }
		...
		for ($i=1;$i<=9;$i++) {
			if (isset($foreign['q_boothcount'.$i]) && $foreign['q_boothcount'.$i] == 1) {
				...
				if (($row['spaceabbr'] != 'A' && $spacecounts['A'] > 0) ||
					...
					$data['message']['__all'] = '<br /><span class="red">A space can not apply at the same time as the other spaces.</span>';
					$result = FALSE;
					break;
				}
				$spacecounts[$row['spaceabbr']] += $row['boothcount'];
				if ($spacecounts[$row['spaceabbr']] > $spacelimits[$row['spaceabbr']]) {
					$data['message']['__all'] = '<br /><span class="red">I have exceeded the maximum number of application booth.</span>';
					$result = FALSE;
					break;
				}
				...
				if ($contcount > 0 && $waitcount > 0) {
					$data['message']['__all'] = '<br /><span class="red">It is not possible to apply simultaneous space of a waiting space and usually accepted.</span>';
					$result = FALSE;
					break;
				}
			}
    ...
    function regist()
    {
        if ($this->check_limit_action()) {
            $this->parser->parse('en/entry_limit');
        } else {
            if ($this->check_auth()) {
                parent::regist();
            } else {
	...
	protected function after_regist(&$data)
	{
		...
		if (isset($data['foreign']['statusno']) && $data['foreign']['statusno'] == 202) {
			$text = $this->parser->parse('mail/en/entry_regist_url_waiting.txt', $data, TRUE);
		} else {
			$text = $this->parser->parse('mail/en/entry_regist_url.txt', $data, TRUE);
		}
		if (strpos($text, "\n") !== FALSE) {
			list($subject, $message) = explode("\n", $text, 2);
		} else {
			$subject = 'TOKYO AUTO SALON 2020 "automatic reply mail"';
			$message = $text;
		}
