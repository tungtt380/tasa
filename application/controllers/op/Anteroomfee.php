...
* 控室請求一覧
*/
class Anteroomfee extends MemOP_Controller
{
    protected $form_prefix = 'anteroomfee';
...
//      $this->db->from('v_billing_anteroomfee_total et');
//      $this->db->join('v_billing_search bb', 'bb.billid = et.mergebillid AND bb.seqno = 0');
        $this->db->from('v_billing_ex_search bb');
		$this->db->join('v_invoice_anteroomfee iv', 'iv.billid = bb.billid AND iv.expired = 0');
...
//              (isset($data['foreign']['itemdetail'.$key]) && $data['foreign']['itemdetail'.$key] != '') ||
        ...
        if (!isset($data['message']) || empty($data['message'])) {
            ...
            foreach($data['foreign']['booths'] as $key) {
                ...
                $data['foreign']['subtotal'] += intval(empty($data['foreign']['unitprice'.$key])?0:$data['foreign']['unitprice'.$key]) * intval(empty($data['foreign']['quantity'.$key])?0:$data['foreign']['quantity'.$key]);
    ...
    function create_record(&$foreign)
    {
        ...
        return $this->invoice_model->createitem($foreign, 31);
    }
...
/* End of file anteroomfee.php */
/* Location: ./application/controllers/office/anteroomfee.php */
