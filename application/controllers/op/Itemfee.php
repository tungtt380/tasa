...
class Itemfee extends MemOP_Controller
{
    protected $form_prefix = 'itemfee';
	...
	function regist_in()
	{
		...
		foreach($data['foreign']['booths'] as $key) {
			if ((isset($data['foreign']['itemname'.$key])   && $data['foreign']['itemname'.$key] != '') ||
        ...
        if (!isset($data['message']) || empty($data['message'])) {
			...
			foreach($data['foreign']['booths'] as $key) {
				...
				// $data['foreign']['subtotal'] += intval($data['foreign']['unitprice'.$key] * $data['foreign']['quantity'.$key]);
                // $data['foreign']['discount'] += intval($data['foreign']['discount'.$key]);
                $data['foreign']['subtotal'] += intval((empty($data['foreign']['unitprice'.$key])?0:$data['foreign']['unitprice'.$key]) * (empty($data['foreign']['quantity'.$key])? 0:$data['foreign']['quantity'.$key]));
	...
	function create_record(&$foreign)
	{
		...
		return $this->invoice_model->createitem($foreign, false, $force_include_tax = true);
	}
