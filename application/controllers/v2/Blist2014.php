...
class Blist2014 extends CI_Controller {
	...
	public function e01carj()
	{
		$db =& $this->db;
		$db->select("CONCAT('2014',LPAD(ed.appid,5,'0')), 'appid', ed.appno, bo.exhboothno", FALSE);
		$db->select("e.exhid, e.corpname, e.brandname");
		$db->select("ed.spec, CASE ed.spec WHEN 0 THEN 'チューニング' WHEN 1 THEN 'ドレスアップ' WHEN 2 THEN 'コンセプト' END 'specname'", FALSE);
		$db->select("ed.carname, ed.carkana");
		$db->select("ed.basecarname, ed.basecartype, basecaryear, ed.boothtype");
		$db->select("ed.sectionno, ed.enableroad");
		$db->select("IF(ed.enablerace,'競技専用','') 'enablerace', IF(ed.prototype,'参考出品','') 'prototype'", FALSE);
		$db->select("ed.sales, ed.stand");
		$db->select("ed.publicdate");
		$db->select("ed.concept, ed.complete, ed.progress", FALSE);
		$db->select("IF(bo.exhboothno,CONCAT(bo.exhboothno,LPAD(ed.seqno,2,'0')),NULL) 'entryno'", FALSE);
		$db->select("ed.enginetype, ed.enginecc");
		$db->select("ed.outputnum, ed.outputunit, ed.outputrpm");
		$db->select("ed.torquenum, ed.torqueunit, ed.torquerpm");
		$db->select("ed.enginecomment", FALSE);
		$db->select("ed.muffler, ed.manifold, ed.transmission");
		$db->select("ed.clutch, ed.differential, ed.aeroname");
		$db->select("ed.bodycolor");
		$db->select("ed.dressupcomment");
		$db->select("ed.sheet, ed.steering, ed.meter");
		$db->select("ed.audio, ed.carnavi");
		$db->select("ed.floormat");
		$db->select("ed.etc", FALSE);
		$db->select("ed.suspension, ed.absorber, ed.spring, ed.brake");
		$db->select("ed.suspensioncomment,", FALSE);
		$db->select("ed.wheel, ed.frontsize, ed.rearsize");
		$db->select("ed.tire, ed.fronttire, ed.reartire");
		$db->select("ed.maxspeed, ed.dragspeed");
		$db->select("ed.speedcomment", FALSE);
		$db->select("ed.comment", FALSE);
		$db->select("CONCAT('http://archive.tokyoautosalon.jp/2014/photos/car/',ed.photo1) 'photo1'", FALSE);
		$db->select("CONCAT('http://archive.tokyoautosalon.jp/2014/photos/car/',ed.photo2) 'photo2'", FALSE);
		$db->select("CONCAT('http://archive.tokyoautosalon.jp/2014/photos/car/',ed.photo3) 'photo3'", FALSE);
...
//		$db->select("ea.zip '(公開)郵便番号', ea.prefecture '(公開)都道府県', ea.address1 '(公開)住所1', ea.address2 '(公開)住所2', ");
		...
		$db->select("CONCAT('http://archive.tokyoautosalon.jp/2014/photos/publicinfo/',ed.photo) '(公開)画像'", FALSE);
...
//      $result = mb_convert_kana($result, "KVsa", "UTF-8");
	...
	public function e03present()
	{
		...
		$db->select("CONCAT('http://archive.tokyoautosalon.jp/2014/photos/present/',ed.itemphoto) '商品画像'", FALSE);
