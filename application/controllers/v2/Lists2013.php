...
class Lists extends CI_Controller {
	...
	protected function wordslike($letter)
	{
		switch($letter){
		case 'a':
			$this->db->collate_like('e.corpkana', 'あ', 'after');
			$this->db->or_collate_like('e.corpkana', 'い', 'after');
			$this->db->or_collate_like('e.corpkana', 'う', 'after');
			$this->db->or_collate_like('e.corpkana', 'え', 'after');
			$this->db->or_collate_like('e.corpkana', 'お', 'after');
			break;
		case 'k':
			$this->db->collate_like('e.corpkana', 'か', 'after');
			$this->db->or_collate_like('e.corpkana', 'き', 'after');
			$this->db->or_collate_like('e.corpkana', 'く', 'after');
			$this->db->or_collate_like('e.corpkana', 'け', 'after');
			$this->db->or_collate_like('e.corpkana', 'こ', 'after');
			break;
		case 's':
			$this->db->collate_like('e.corpkana', 'さ', 'after');
			$this->db->or_collate_like('e.corpkana', 'し', 'after');
			$this->db->or_collate_like('e.corpkana', 'す', 'after');
			$this->db->or_collate_like('e.corpkana', 'せ', 'after');
			$this->db->or_collate_like('e.corpkana', 'そ', 'after');
			break;
		case 't':
			$this->db->collate_like('e.corpkana', 'た', 'after');
			$this->db->or_collate_like('e.corpkana', 'ち', 'after');
			$this->db->or_collate_like('e.corpkana', 'つ', 'after');
			$this->db->or_collate_like('e.corpkana', 'て', 'after');
			$this->db->or_collate_like('e.corpkana', 'と', 'after');
			break;
		case 'n':
			$this->db->collate_like('e.corpkana', 'な', 'after');
			$this->db->or_collate_like('e.corpkana', 'に', 'after');
			$this->db->or_collate_like('e.corpkana', 'ぬ', 'after');
			$this->db->or_collate_like('e.corpkana', 'ね', 'after');
			$this->db->or_collate_like('e.corpkana', 'の', 'after');
			break;
		case 'h':
			$this->db->collate_like('e.corpkana', 'は', 'after');
			$this->db->or_collate_like('e.corpkana', 'ひ', 'after');
			$this->db->or_collate_like('e.corpkana', 'ふ', 'after');
			$this->db->or_collate_like('e.corpkana', 'へ', 'after');
			$this->db->or_collate_like('e.corpkana', 'ほ', 'after');
			break;
		case 'm':
		case 'y':
		case 'r':
		case 'w':
			$this->db->collate_like('e.corpkana', 'ま', 'after');
			$this->db->or_collate_like('e.corpkana', 'み', 'after');
			$this->db->or_collate_like('e.corpkana', 'む', 'after');
			$this->db->or_collate_like('e.corpkana', 'め', 'after');
			$this->db->or_collate_like('e.corpkana', 'も', 'after');
			$this->db->or_collate_like('e.corpkana', 'や', 'after');
			$this->db->or_collate_like('e.corpkana', 'ゆ', 'after');
			$this->db->or_collate_like('e.corpkana', 'よ', 'after');
			$this->db->or_collate_like('e.corpkana', 'ら', 'after');
			$this->db->or_collate_like('e.corpkana', 'り', 'after');
			$this->db->or_collate_like('e.corpkana', 'る', 'after');
			$this->db->or_collate_like('e.corpkana', 'れ', 'after');
			$this->db->or_collate_like('e.corpkana', 'ろ', 'after');
			$this->db->or_collate_like('e.corpkana', 'わ', 'after');
			$this->db->or_collate_like('e.corpkana', 'を', 'after');
			$this->db->or_collate_like('e.corpkana', 'ん', 'after');
			break;
		}
		$this->db->order_by('e.corpkana');
	}
	...
	public function exhibitors($word='')
	{
        ...
        $this->load->dbutil();
        $this->db->select("e.exhid '出展コード', e.corpname '出展者名', e.corpkana '出展者名カナ'");
        ...
        $this->db->select("e.position '役職', e.fullname '代表者氏名', e.fullkana '代表者氏名カナ'");
	...
	public function exhibitor_booths($word='')
	{
        ...
        $this->load->dbutil();
        $this->db->select("bo.exhboothno '小間番号'");
        $this->db->select("sp.spaceabbr 'スペース', bs.boothabbr '小間形状'");
	...
	public function e01car()
	{
		...
		$this->db->select("e.exhid '出展者コード', e.corpname '出展者名', e.brandname '表示名'");
		$this->db->select("ed.spec '系番号', CASE ed.spec WHEN 0 THEN 'チューニング' WHEN 1 THEN 'ドレスアップ' WHEN 2 THEN 'コンセプト' END '系登録'", FALSE);
		$this->db->select("ed.carname '出展車両名', ed.carkana '出展車両名カナ'");
		...
		$this->db->select("ed.sales '販売可能', ed.stand 'スペックボードの作成'");
		$this->db->select("ed.publicdate '公開日'");
		$this->db->select("REPLACE(ed.concept,'\\n','\\\\n') 'コンセプト', IF(ed.complete>0, 100, ed.progress) AS '完成度'", FALSE);
		$this->db->select("IF(bo.exhboothno,CONCAT(bo.exhboothno,LPAD(ed.seqno,2,'0')),NULL) '車両番号'", FALSE);
		...
		$this->db->select("CONCAT('http://archive.tokyoautosalon.jp/2012/photos/car/',ed.photo1) 'photo1'", FALSE);
		...
		$this->db->select("ed.updated '最終更新日時'");
        $this->db->select("e.promotion 'プロモーションコード'");
...
//      $result = mb_convert_kana($result, "KVsa", "UTF-8");
	...
	public function e01carex()
	{
        ...
        $this->db->where('ed.expired', '0');
        $this->db->where('bo.expired', '0');
		$this->db->where("(`publicdate` IS NULL OR STR_TO_DATE(`publicdate`,'%Y-%m-%d %H:%i:%s') <= CURRENT_TIMESTAMP)", NULL, FALSE);
        $this->db->where('e.expired', '0');
        $this->db->where_in('e.statusno', array('500','401','400'));
        $query = $this->db->get();
        if ($query === FALSE) {
...
//      $result = mb_convert_kana($result, "KVsa", "UTF-8");
	...
	{
		$this->db->select("ea.appid '手続番号', ea.appno '手続書類', bo.exhboothno '小間番号'");
		$this->db->select("e.exhid '出展者コード', e.corpname '出展者名', e.brandname '表示名'");
		$this->db->select('ea.publicaddress 住所公開, ea.publicphone TEL公開, ea.publicfax FAX公開, ea.publicurl URL公開, ea.publicemail メールアドレス公開');
		$this->db->select("e.countrycode AS '国'");
		$this->db->select("ea.zip '(公開)郵便番号', ea.prefecture '(公開)都道府県', ea.address1 '(公開)住所1', ea.address2 '(公開)住所2', ");
		$this->db->select("ea.phone '(公開)電話番号', ea.fax '(公開)FAX番号', ea.email '(公開)メールアドレス'");
		$this->db->select("CONCAT('http://',REPLACE(ea.url,'http://','')) '(公開)URL'",FALSE);
		$this->db->select("REPLACE(ea.prcomment,'\\n','\\\\n') 'PRコメント'", FALSE);
		$this->db->select("CONCAT('http://archive.tokyoautosalon.jp/2012/photos/publicinfo/',ed.photo) '(公開)画像'", FALSE);
		$this->db->select("REPLACE(ed.photocomment,'\\n','\\\\n') '(公開)画像コメント'", FALSE);
        $this->db->select("e.promotion 'プロモーションコード'");

        $this->db->from('v_exapply_02 ea');
        $this->db->join('v_exapply_02_detail ed', 'ea.exhboothid = ed.exhboothid', 'left');
        $this->db->join('exhibitor_booth bo', 'ea.exhboothid = bo.exhboothid');
        $this->db->join('exhibitors e', 'e.exhid = bo.exhid');
        $this->db->join('booths bs', 'bs.boothid = bo.boothid');
        $this->db->where('bo.expired', '0');
        $this->db->where('e.expired', '0');
...
//      $result = mb_convert_kana($result, "KVsa", "UTF-8");
	...
	public function e03present()
	{
		$this->db->select("ea.appid '手続番号', ea.appno '手続書類', bo.exhboothno '小間番号'");
		$this->db->select("e.exhid '出展者コード', e.corpname '出展者名', e.brandname '表示名'");
		$this->db->select("ea.corpname '(協賛)企業名'");
		$this->db->select("ea.zip '(協賛)郵便番号', ea.prefecture '(協賛)都道府県', ea.address1 '(協賛)住所1', ea.address2 '(協賛)住所2', ");
		$this->db->select("ea.phone '(協賛)電話番号', ea.fax '(協賛)FAX番号'");
		$this->db->select("ed.seqno '商品番号', ed.itemname '商品名', ed.itemprice '標準価格', ed.quantity '個数'");
		$this->db->select("CONCAT('http://archive.tokyoautosalon.jp/2012/photos/present/',ed.itemphoto) '商品画像'", FALSE);
        $this->db->from('v_exapply_03 ea');
        $this->db->join('v_exapply_03_detail ed', 'ea.exhboothid = ed.exhboothid', 'left');
        $this->db->join('exhibitor_booth bo', 'ea.exhboothid = bo.exhboothid');
        $this->db->join('exhibitors e', 'e.exhid = bo.exhid');
        $this->db->join('booths bs', 'bs.boothid = bo.boothid');
        $this->db->where('ed.expired', '0');
