		...
		while ($i < $nb) {
			...
			if ($c == 10) { // 10 = "\n" = new line
				...
				if (($c != 160) AND (($c == 173) OR preg_match($this->re_spaces, $this->unichr($c)))) {
					...
					if (mb_detect_encoding($txt, "ASCII, UTF-8", TRUE) == "UTF-8") {
						$sep = -1;
					} else {
//						$sep = $i;
						// check if is a SHY
	...
	protected function _addfield($type, $name, $x, $y, $w, $h, $prop) {
		...
		// Upgrade PHP7 - Silence “The each() function is deprecated” error in PHP 7 - Start by TTM
		// while (list($key, $val) = each($prop)) {
		foreach($prop as $key => $val ) {
		// Upgrade PHP7 - Silence “The each() function is deprecated” error in PHP 7 - End by TTM
